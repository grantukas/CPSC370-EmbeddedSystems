...
int main(void)
{
  ...
  osMessageQDef(messageQueue, 1, uint16_t);
...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
	  GPIO_PinState blueButton = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);
	  ...
	  if(blueButton == 1){
		  ...
		  if(alarmState == 0){
			  ...
			  //osDelay(500); //Check to see Orange on first
		  } else {
			  alarmState = 0;
			  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, GPIO_PIN_RESET);
		  }

		  osMessagePut(messageQueueHandle, alarmState, 500);
		  buttonState = 0;
		  osDelay(250);
	  }
  }
...
void StartAlarmTask(void const * argument)
{
  ...
  for(;;)
  {
	  osEvent QreadState = osMessageGet(messageQueueHandle, 500);

	  if(QreadState.status == osEventMessage) {
		  uint32_t SensorStateMessage = QreadState.value.v;

		  if(SensorStateMessage == 1){
